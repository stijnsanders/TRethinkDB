...
type
  ...
  public
    ...
    function SendBuf(const Buffer; Count: Cardinal): Cardinal;
...
function TTcpSocket.SendBuf(const Buffer; Count: Cardinal): Cardinal;
var
  p:pointer;
begin
  p:=@Buffer;
  Result:=send(FSocket,p^,Count,0);
  if Result=Cardinal(SOCKET_ERROR) then
   begin
    Disconnect;
    RaiseLastWSAError;
   end;
